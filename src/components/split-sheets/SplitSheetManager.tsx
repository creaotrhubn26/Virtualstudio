...
export default function SplitSheetManager({
  ...
  const handleEditorSave = (splitSheet: SplitSheet) => {
    ...
    // Update selectedSplitSheet with the saved split sheet (which now has an ID)
    // This allows the editor to show the updated split sheet with ID
    setSelectedSplitSheet(splitSheet);
    
    // Only close editor if it was an update (not a new creation)
    // For new split sheets, keep editor open so user can see the ID and use portal view
    if (selectedSplitSheet?.id) {
      setShowEditor(false);
      setSelectedSplitSheet(null);
    }
